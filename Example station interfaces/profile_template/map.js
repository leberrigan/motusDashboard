	...
	motusMap = {
		...
		setVisibility: function(switchType) {
			...
			if (switchType === true && motusMap.svg !== 'undefined') {
				...
				dataType = ['tracks','points'].includes(mapType) && exploreType == 'species' ? 'species' : 'stations'
			...
			if (motusMap.svg !== 'undefined') {
				...
				motusMap.svg.selectAll(".explore-map-" + mapType + ".explore-map-" + dataType)
					.classed('hidden', d => motusMap.isVisible(d));
		...
		*/
		dataHover: function(e, d, dir, t){
			if (dir == 'in') {
				...
				if (t == 'track') {
					...
					$('.tooltip').html(
						"<big>"+ 
							(filterName != 'projID' ? motusData.species[d.species] : "Project " + d.projID)+ 
						"</big>"+
						(filterName != 'species' ? ("<br/>Species: " + motusData.species[d.species]) : "") +
		...
		setColour: function(val) {
			...
			if (val != 'undefined') {
				...
				console.log(motusMap.colorScales[dataType][mapType][val]("Conestogo"));
...
function loadMapObjects(callback) {
	...
	for (dataset in motusData) {
		...
		if (dataset == 'regions') {
			...
			motusMap.regionColours = d3.group(motusData.regions, d => d.code);
			...
			motusData.stations.forEach(function(row){
				...
				row.dtStart = new Date(row.dtStart);
				row.dtEnd = row.dtEnd == "NA" ? new Date() : new Date(row.dtEnd);
				
				row.lastData = (new Date().getTime() - row.dtEnd) / (1000 * 60 * 60 * 24);
		...
		//	console.log(all_stations);
			...
			motusMap.colorScales.stations.points.name = d3.scaleOrdinal().domain(all_stations).range(customColourScale.jnnnnn.slice(0, all_stations.length));
	
			console.log(motusMap.colorScales.stations.points.name(3237));
			...
			motusMap.colorScales.stations.points.projID = d3.scaleOrdinal().domain($("#filter_projects option:not(:first-child)").map(function(){return this.value;}).get()).range(customColourScale.jnnnnn.slice(0, nProjs));
			motusMap.colorScales.species.points.projID = d3.scaleOrdinal().domain($("#filter_projects option:not(:first-child)").map(function(){return this.value;}).get()).range(customColourScale.jnnnnn.slice(0, nProjs));
			motusMap.colorScales.species.tracks.projID = d3.scaleOrdinal().domain($("#filter_projects option:not(:first-child)").map(function(){return this.value;}).get()).range(customColourScale.jnnnnn.slice(0, nProjs));
			
			motusMap.colorScales.species.tracks.species = d3.scaleOrdinal().domain($("#filter_species option:not(:first-child)").map(function(){return this.value;}).get()).range(customColourScale.jnnnnn.slice(0, $("#filter_species option:not(:first-child)").length));
			motusMap.colorScales.species.points.species = motusMap.colorScales.species.tracks.species;
			
			
			motusMap.colorScale = motusMap.colorScales.stations.points.name;
			...
			var station_el = g.selectAll("stations")
				...
				.style("stroke", "#000")
				.style('fill', (d) => motusMap.colorScale(d.name))
		...
		*/
			...
			motusData.tagDeps.forEach(function(row){
				row.status = row.status == 'active' ? (row.dtEnd == 'NA' ? 'active' : 'expired') : row.status;
				topush = {
					type: "Point", 
			...
			var tagDepsSpecies = d3.rollup(motusData.tagDeps, v => v.length, d => d.species);
...
//			console.log(Array.from(tagDepsSummary.values()))
			...
			motusData.tracks.forEach(function(row){
			...
			motusMap.colorScales.species.tracks.id = d3.scaleOrdinal().domain(trackDataLink.map( x => +x.id ).filter(onlyUnique)).range(customColourScale.jnnnnn.slice(0, nProjs));
			/*
			motusMap.colorScales.species.tracks.species = d3.scaleOrdinal().domain(motusData.species.map( x => +x.id ).filter(onlyUnique)).range(customColourScale.jnnnnn.slice(0, motusData.species.length));*/
			...
			// Add the path
			var tracks_el = g.selectAll("tracks")
				.data(trackDataLink)
				.enter().append("path")
	...
	function reset() {
		var bounds = path.bounds(motusData.polygons),
			topLeft = bounds[0],

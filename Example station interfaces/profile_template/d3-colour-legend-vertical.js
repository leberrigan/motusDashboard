...
function legendVertical({
  ...
  if (color.interpolate) {
    ...
    y = color
      .copy()
      .rangeRound(
        d3.quantize(d3.interpolate(marginTop, height - marginBottom), n)
      );
  ...
  // Sequential
  else if (color.interpolator) {   console.log('test2');
    y = Object.assign(
      color
        .copy()
    ...
    svg
      ...
      .attr("height", height - marginTop - marginBottom)
      .attr("preserveAspectRatio", "none")
      .attr("ylink:href", vRamp(color.interpolator(), 256, invert).toDataURL());
  ...
  // Ordinal
    ...
    svg
		...
		.on('mouseover', function(e, d){
			...
			map.svg.selectAll(".explore-map-"+dataType+".explore-map-"+mapType+":not(.hidden)").style('fill-opacity', 0).style('opacity',0);
			
			d = String(d).split(',');
			
			map.svg.selectAll(".explore-map-"+dataType+".explore-map-"+mapType+":not(.hidden)").filter(function(data) {
				return data[motusFilter.colour].split(',').filter(x => d.includes(x)).length > 0;
...
function vRamp(color, n = 256, invert) {

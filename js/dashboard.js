...
function loadCards() {
	var toAppend, header, icon, widgets;
	for (item in pageContent) {
		header = pageContent[item].header;
		icon = pageContent[item].icon;
		iconColor = pageContent[item].iconColor;
		toAppend = "<div id='card_" + item + "'><div><img class='" + (iconColor != 'undefined' ? iconColor : '') + "' src='" + icon + "' alt='" + header + "'><h3>" + header + "</h3></div><div class='widgets'></div></div>";
		$("#card-menu").append(toAppend);
		widgets = pageContent[item].widgets;
		if (widgets != undefined && widgets.length > 0) {
			for (w in widgets) {
				var widget = widgets[w];
				toAppend = "<div id='#card_" + item + "_widget" + w + "' class='widget"+(widget.priority == undefined? "" : " " + widget.priority) + "'>";
//				toAppend += ;
				if (widget.type == "text" || widget.type == "number") {
					var value = widget.value.split(",");
					console.log(value.length);
					toAppend += "<div><span" + (widget.type == "text" ? " class='small'" : "") + ">" + value[0] + (value.length > 1 ? "/</span>" + value[1] : "</span>") + "</div></div>";
				}
				$("#card_" + item + " .widgets").append(toAppend);
			}
		}
	}
	
	$("#card-menu_btn").click(function(){$("#card-menu").toggleClass("hidden");});
	
	$("#card-menu > div").each(loadContentHTML).click(showContent);
	console.log("$(\"#card_"+currentPage+"\").length = " + $("#card_" + currentPage).length);
	if ($("#card_" + currentPage).length > 0) {
		$("#card_" + currentPage).click();
	} else {$("#card-menu > div").eq(0).click();}
}
...
function loadContentHTML() {
	...
	if (page != 'main') {
		...
		$("#main-content").append("<div id='" + page + "-content' class='item-content'>"+
												"<input type='hidden' value='0' id='" + page + "-data_isLoaded'>"+
												"<table class='main-table'><tbody></tbody></table>"+
												"</div>");
		//		console.log(this.id);
												
		for (el_id in pageContent[page].elements) {
			//	console.log(el_id);
			el = pageContent[page].elements[el_id];
			if (el.type == 'tabs') {
				$("#" + page + "-content").append("<div id='" + page + delimiter + el_id + "'><ul></ul></div>");
				for (tab_id in el.tabs) {
					tab = el.tabs[tab_id];
					$("#"  + page + delimiter + el_id + " ul").append("<li><a href='#" + page + delimiter + el_id + delimiter + tab_id + "'>" + tab.name + "</a></li>");
					$("#" + page + delimiter + el_id).append("<div id='" + page + delimiter + el_id + delimiter + tab_id + "'></div>");
					for (e_id in tab.elements) {
						e = tab.elements[e_id];
						if (e.type == 'input') {
			//				addInputElement($("#"  + page + "-" + el_id + "-" + tab_id + "-content"));
							addInputElement($("#"  + page + delimiter + el_id + delimiter + tab_id), page + delimiter + el_id + delimiter + tab_id + delimiter + e_id, e.input, e.label);
						} else if (e.type == 'table') {
							addTable($("#"  + page + delimiter + el_id + delimiter + tab_id), page + delimiter + el_id + delimiter + tab_id + delimiter + e_id, e.cols);
						} else if (e.type == 'map') {
							addMap($("#"  + page + delimiter + el_id + delimiter + tab_id),  page + delimiter + el_id + delimiter + tab_id + delimiter + e_id, e);
						}
					}
				}
				$("#" + page + delimiter + el_id).tabs();
			} else if (el.type == 'table') {
				addTable($("#" + page + "-content"), page + delimiter + el_id, el.cols);
			} else if (el.type == 'input') {
				addInputElement($("#" + page + "-content .main-table tbody"), page + delimiter + el_id, el.input, el.label);
			} else if (el.type == 'map') {
				addMap($("#" + page + "-content .main-table"), page + delimiter + el_id, el);
			}
		}
...
function showContent() {
	if (!$(this).hasClass('selected')) {
		$("#card-menu.collapsed:visible #card-menu_btn").click();
		var page = this.id.replace("card_","");
		if (page != "main" && !$("#card-menu").hasClass('collapsed')) {
			$("#card-menu").addClass('collapsed');
		} else if (page == "main" && $("#card-menu").hasClass('collapsed')) {
			$("#card-menu").removeClass('collapsed');
			$(".item-content:visible").hide();
		}
		currentPage = page
		window.history.pushState("test", "test", setUrlParameter(window.location.href, 'page', currentPage));
		$("#card-menu > div.selected").removeClass('selected');
		$(this).addClass('selected');
		
		if ($("#" + page + "-content:hidden").length > 0) {
			
			$(".item-content:visible").hide();
			
			if ($("#" + page + "-data_isLoaded").val() == 0) {
				loadData(page);
			}
